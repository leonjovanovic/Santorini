...
public class SimpleAI extends Player implements Cloneable {
	...
	public void function() {
		...
		if (table.sant.mode == 2 && !flag) {// EvE prva figurica random
			...
			if (table.get_field(x_func, y_func).is_taken())
				function();
			flag = true;
			return;
		}
		...
		while (table.get_field(x_func, y_func).is_taken()) {
			if (y)
				if (x_func + 1 < 5 && x_func + 2 < 5)
					x_func++;
				else
					x_func--;
			if (x)
				if (y_func + 1 < 5 && y_func + 2 < 5)
					y_func++;
				else
					y_func--;
			if (!x && !y) {
	...
	private class FieldAction implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (table.p11 && table.sant.mode == 2 && id == 2) {
				...
				table.ai1.function();
				table.ai1.create_figures2(table.ai1.x_func, table.ai1.y_func);
				...
				table.p12 = false;
				table.text_file.println(table.encrypt(table.ai1.f1.f.y, table.ai1.f1.f.x) + " "
						+ table.encrypt(table.ai1.f2.f.y, table.ai1.f2.f.x));
				return;
			}
			if (table.p21 && !table.p12) {
				...
				table.p22 = false;
				table.text_file.println(table.encrypt(table.ai2.f1.f.y, table.ai2.f1.f.x) + " "
						+ table.encrypt(table.ai2.f2.f.y, table.ai2.f2.f.x));
			...
			if (table.sant.mode == 1 && table.player1 == 2 && id == 2) {// Za PvE
				...
				table.sant.l2
						.setText("<html> Player 2 <br> Best score: <br> " + Math.round(tree.root.f_score) + " </html>");
				Node next = tree.next_root();// Tabla sa sledecim odigranim potezom;
				tree = null;
				table.repaint(next.table);
				table.sant.stanje
						.setText(" State: Player " + table.player1 + " is on move. Previous player moved figure "
								+ table.sant.coordM + " and built on " + table.sant.coordB);
				// *****Uslovi pobede i poraza******
				if (table.ai2.f1.isWinner() || table.ai2.f2.isWinner()) {
					JOptionPane.showMessageDialog(table.sant, "Player 2 is winner!");
					table.sant.dispose();
					table.sant.reset();
					table.p1.reset();
					table.ai2.reset();
				...
				if (table.get_player(2).isLoser()) {
					JOptionPane.showMessageDialog(table.sant, "Player 2 is loser!");
					table.sant.dispose();
					table.sant.dispose();
					table.sant.reset();
					table.p1.reset();
			...
			if (table.sant.mode == 2) {// Za EvE
				...
				System.out.println("Najbolji score: " + tree.root.f_score);
				if (table.player1 == 1)
					table.sant.l1.setText(
							"<html> Player 1 <br> Best score: <br> " + Math.round(tree.root.f_score) + " </html>");
				else
					table.sant.l2.setText(
							"<html> Player 2 <br> Best score: <br> " + Math.round(tree.root.f_score) + " </html>");
				Node next = tree.next_root();// Tabla sa sledecim odigranim potezom;
				tree = null;
				System.gc();// Hintujemo Garbage collectoru da imamo visece reference
				table.repaint(next.table);
				System.out.println("Player 12 je na: "+table.find_figure(1, 2).x+" - "+table.find_figure(1, 2).x);
				table.sant.validate();
				table.sant.stanje
						.setText(" State: Player " + table.player1 + " is on move. Previous player moved figure "
								+ table.sant.coordM + " and built on " + table.sant.coordB);
				...
				if (table.ai1.f1.isWinner() || table.ai1.f2.isWinner()) {
					System.out.println(
							"Prosecno vreme minimaxa: " + durationSum / 1000 / sum + " a broj cvorova je " + sum);
					if (table != null)
						table.text_file.close();
					table.sant.end_game = true;
					JOptionPane.showMessageDialog(table.sant, "Player 1 is winner!");
					table.sant.dispose();
					table.sant.reset();
					table.ai1.reset();
					table.ai2.reset();
	...
	public double minimax(Node node, int depth, boolean isMaximizingPlayer) {
		...
		if (node.children == null || depth == table.sant.diff) {// children.isEmpty()
			return node.f_score;
		}
