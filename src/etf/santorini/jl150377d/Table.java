...
public class Table implements Cloneable {
	...
	public boolean building_tree = false;
	...
	public Field find_field(Field field) {
		for (int i = 0; i < 5; i++)
			for (int j = 0; j < 5; j++) {
				if (lista[i][j].x == field.x && lista[i][j].y == field.y)
					return lista[i][j];
			}
		return null;
	}
	...
	public Table clone() throws CloneNotSupportedException {
		...
		if (this.p1 != null) {
			table.p1 = p1.clone();
			table.p1.table = table;
			table.p1.f1.table = table;
			table.p1.f2.table = table;
			table.p1.f1.f = table.get_field(p1.f1.y, p1.f1.y);
			table.p1.f2.f = table.get_field(p1.f2.y, p1.f2.y);
		}
		if (this.p2 != null) {
			table.p2 = p2.clone();
			table.p2.table = table;
			table.p2.f1.table = table;
			table.p2.f2.table = table;
			table.p2.f1.f = table.get_field(p2.f1.y, p2.f1.y);
			table.p2.f2.f = table.get_field(p2.f2.y, p2.f2.y);
		}
		if (this.ai1 != null) {
			table.ai1 = ai1.clone();
			table.ai1.table = table;
			table.ai1.f1.table = table;
			table.ai1.f2.table = table;
			table.ai1.f1.f = table.get_field(ai1.f1.y, ai1.f1.y);
			table.ai1.f2.f = table.get_field(ai1.f2.y, ai1.f2.y);
		}
		if (this.ai2 != null) {
			table.ai2 = ai2.clone();
			table.ai2.table = table;
			table.ai2.f1.table = table;
			table.ai2.f2.table = table;
			table.ai2.f1.f = table.get_field(ai2.f1.y, ai2.f1.y);
			table.ai2.f2.f = table.get_field(ai2.f2.y, ai2.f2.y);
		}
		for (int i = 0; i < 5; i++)
			for (int j = 0; j < 5; j++) {
				table.lista[i][j].table = table;
				if (this.lista[i][j].figure != null)
					table.lista[i][j].figure = this.lista[i][j].figure;
				else
					table.lista[i][j].figure = null;
			}

		if (this.sant.mode != 2) {
			table.current_player = this.current_player.clone();
			table.origin = this.origin.clone();
		}
	...
	public void repaint(Table table) {
		...
		for (int i = 0; i < 5; i++)
			for (int j = 0; j < 5; j++) {
				if (this.lista[i][j].cur_height != table.lista[i][j].cur_height) {
					...
					this.lista[i][j].button.setText(
							this.lista[i][j].cur_height + " / " + this.lista[i][j].id + "" + this.lista[i][j].id2);
		...
		if (ai1 != null && ((ai1.f1.x != table.ai1.f1.x || ai1.f1.y != table.ai1.f1.y)
				|| (ai1.f2.x != table.ai1.f2.x || ai1.f2.y != table.ai1.f2.y))) {
			...
			if (table.sant.load == 0 && (ai1.f1.x != table.ai1.f1.x || ai1.f1.y != table.ai1.f1.y)) {
				...
				sant.coordM = "11 on field " + encrypt(table.ai1.f1.y, table.ai1.f1.x);
			} // Upisi u save.txt odakle polazi i gde se pomera za figuru 1
			f1.remove_figure();
			...
			ai1.f1.f = f1;
			ai1.f1.cur_height = f1.cur_height;
			...
			ai1.f2.f = f2;
			ai1.f2.cur_height = f2.cur_height;
			f2.add_figure(ai1.f2);
			f2.button.setText(f2.cur_height + " / " + f2.id + "" + f2.id2);
		}
		if (ai2 != null && ((ai2.f1.x != table.ai2.f1.x || ai2.f1.y != table.ai2.f1.y)
				|| (ai2.f2.x != table.ai2.f2.x || ai2.f2.y != table.ai2.f2.y))) {
			...
			ai2.f1.f = f1;
			ai2.f1.cur_height = f1.cur_height;
			...
			ai2.f2.f = f2;
			ai2.f2.cur_height = f2.cur_height;
			f2.add_figure(ai2.f2);
			f2.button.setText(f2.cur_height + " / " + f2.id + "" + f2.id2);
		}
		if (flag) {
			text_file.println(encrypt(y, x));
			flag = false;
			sant.coordB = encrypt(y, x);
		}
	}
}
