...
public class Table implements Cloneable{
	...
	public Table(Santorini s) {
		...
		sant=s;
		try {
			if(s.load==0) text_file = new PrintWriter("save.txt");
	...
	public Table clone() throws CloneNotSupportedException {
		...
		for(int i=0;i<5;i++)
			for(int j=0;j<5;j++) {
				...
				if(this.lista[i][j].figure!=null)table.lista[i][j].figure=this.lista[i][j].figure;
				else table.lista[i][j].figure=null;
				}
